...
import type { QuizPayload, QuizResult } from "@/lib/api/types";
...
export function QuizSection({ lessonId }: { lessonId: string }) {
  ...
  const {
    ...
    error: quizError,
    callApi: fetchQuiz,
  } = useAPI<QuizPayload>({ url: `/quiz/${lessonId}`, lazy: true });
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      const res = await client.post<QuizResult>(`/quiz/${lessonId}/submit`, {
  ...
  if (quizError) {
    const message =
      (quizError as { message?: string | string[] })?.message ??
      "Quiz load হয়নি।";
    return (
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-slate-800">
          🎯 Quiz Time! (pass: {quiz.passPercent}%)
        </h2>
            ...
            {result.passed ? (
              ...
              <>
                <p className="mt-1 text-sm text-rose-700">
                  Pass করতে {result.passPercent}% লাগবে। ভুলগুলোর explanation
                  পড়ে আবার চেষ্টা করো — নতুন প্রশ্ন আসবে!
                </p>
                <CustomButton
                  variant="primary"
